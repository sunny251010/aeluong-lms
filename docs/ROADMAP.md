...
- Đã tạo GitHub Actions deploy thủ công qua `.github/workflows/deploy.yml`.
...
8. Test GitHub Actions deploy thủ công bằng `workflow_dispatch` với `source_path = wp-content` và `deployment_dest = wp-content`.
